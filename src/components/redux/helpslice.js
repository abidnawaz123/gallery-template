...
const apiSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            //updateHeightOfItem
            .addCase(updateLocalData, (state, action) => {
                state.tag = state.tag[action.payload.id].length = action.payload.length
                state.loading = 'succeeded';
                // const toUpdateItem = state.data.filter((item, ind) => item.id == action.payload.id)
                // state.data = action.payload;
            })
